...
get_sfs_peter1(int *total_neu,int *total_sel,int *nalleles,int *sfs_sel,int *sfs_neu,char *sfs_filename)
{
  ...
  fscanf (file, "%d", total_sel);
  sum=0;
  for (i =1 ;i<*nalleles; i++)
    {
      fscanf (file, "%d", &sfs_sel[i]);
      //printf("%d\n",sfs_sel[i]);
      sum+=sfs_sel[i];
    }
  sfs_sel[0]=*total_sel-sum;
  sfs_sel[*nalleles]=0;
  fscanf (file, "%d", total_neu);
  sum=0;
  for (i =1 ;i<*nalleles; i++)
    {
      fscanf (file, "%d", &sfs_neu[i]);
      //printf("%d\n",sfs_neu[i]);
      sum+=sfs_neu[i];
    }
  sfs_neu[0]=*total_neu-sum;
  sfs_neu[*nalleles]=0;
  //printf("%d\n",sfs_neu[*nalleles]);
  //monitorinput();
  fclose(file);
...
get_sfs_peter2(int *total_neu,int *total_sel,int *nalleles,int *sfs_sel,int *sfs_neu,char *sfs_filename,int *sel_sites,int *sel_diff,int *neu_sites,int *neu_diff)
{
  ...
  fscanf (file, "%s", name);
  fscanf (file, "%d", sel_sites);
  fscanf (file, "%d", sel_diff);
  fscanf (file, "%d", neu_sites);
  fscanf (file, "%d", neu_diff);
  fscanf (file, "%d", nalleles);

  //printf("%s %d %d %d %d %d\n",name,sel_sites,sel_diff,neu_sites,neu_diff,*nalleles);

  fscanf (file, "%d", &sfs_sel[0]);
  //printf("%d\n",sfs_sel[0]);
  sum=0;
  for (i =1 ;i<=*nalleles; i++)
    {
...
/******************************************************************************/
double sfsfold_f(int nalleles,double *sfs,double *sfs_folded,int index)
{
...
int i;
	...
	{
	  sfs_folded[index+i]=sfs[i]+sfs[index-i]; 
...
{
   double s_lower, s_upper, p_upper;
   int i, s_ind;
   s_lower = undefined;
   s_upper = undefined;
...
//   for (i=1; i<n_s_evaluated; i++)
//   {
//      printf("s %lf gamma_density_vec[%d] %lf\n", s_evaluated_vec[i], i,
//         gamma_density_vec[i]);
//   }
//   monitorinput();
return (1);
...
int set_up_density_vec_step_effects(double alpha, double beta, 
				    int n_s_evaluated, double *gamma_density_vec)
{
...
// gamma_density_vec[0]+=1.0-area;
 
 //if (gamma_density_vec[0]>0){printf("%f\n",gamma_density_vec[0]);}
  return (1);
}
/******************************************************************************/
